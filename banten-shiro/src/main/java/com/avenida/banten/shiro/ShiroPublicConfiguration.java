...
public class ShiroPublicConfiguration {
  ...
  public ShiroFilterFactoryBean shiroFilterFactory(
      final Environment environment, final ShiroConfiguration shiroConfig) {
    ...
    shiro.setUnauthorizedUrl(shiroConfig.getUnauthorizedUrl());
    shiro.setSuccessUrl(shiroConfig.getSuccessUrl());
    shiro
        .setFilterChainDefinitionMap(createFilterChainDefinitionMap(shiroConfig
            .getUrlRoleMappings()));
  ...
  /**
   * Gets the security manager. Search in the external config property if it's
   * present a shiro.ini path if it's not, fallbacks to "shiro-dev.ini".
   *
   * @param environment
   *          the Environment.
  ...
  public SecurityManager securityManager(final Environment environment) {
    ...
    String externalShiroConfigPath = environment
        .getProperty(SHIRO_EXTERNAL_CFG);
